...
for(j in 1:N){
  ...
  for(i in 1:q){
   X[,i] <- rbinom(n,1,0.5) #Für Generierung von Dummies
  }
  
  #XXX <- sample(1:(q+1),n,replace = T) #Für Generierung von Faktoren
  #XXXX <- as.factor(XXX)
  #X <- model.matrix(XXX~XXXX)[,-1]
...
plot(lamlist2, MSEr, type = "l", ylim = c(0,maxmse), main = paste("Ridge Faktoren, n = ",n,", N = ",N, sep = ""),
     col = "#440154", lwd = 3, xlab = "log(Lambda)", ylab = "MSE")
lines(lamlist2, v_r, col = "#21918c", lwd = 2.5)
lines(lamlist2, bi2_r, col = "#fde725", lwd = 2.5)


plot(lamlist2, MSEl, type = "l", ylim = c(0,maxmse), main = paste("Lasso Faktoren, n = ",n,", N = ",N, sep = ""),
     col = "#440154", lwd = 3, xlab = "log(Lambda)", ylab = "MSE")
lines(lamlist2, v_l, col = "#21918c", lwd = 2.5)
lines(lamlist2, bi2_l, col = "#fde725", lwd = 2.5)


plot(lamlist2, MSEel, type = "l", ylim = c(0,maxmse), main = paste("EN 0.5 Faktoren, n = ",n,", N = ",N, sep = ""),
     col = "#440154", lwd = 3, xlab = "log(Lambda)", ylab = "MSE")
...
plot(lamlist2, MSEr, type = "l", ylim = c(0,maxmse), col = "#1F77B4", lwd = 2.5,
     main = paste("Vergleich MSE Faktoren, n = ",n,", N = ",N, sep = ""), xlab = "log(Lambda)", ylab = "MSE")
...
x1 <- rbinom(1000,1,0.75)
x2 <- rbinom(1000,1,0.5)
xx <- data.frame(x1,x2)

sds <- sqrt(colMeans(xx^2) - colMeans(xx)^2)
xxt <- t((t(xx)-colMeans(xx))/sds)

et <- xxt%*%c(1,2)
pet <- plogis(et)
yet <- rbinom(1000,1,pet)

glm(yet~., data = xx, family = "binomial")

f <- glmnet(xx,yet, family = "binomial", lambda = c(0,0.0001))
coef(f)
