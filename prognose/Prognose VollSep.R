...
min_measure <- array(NA,dim=c(6,N))
...
for(j in 1:N){
  ...
  min_measure[4,j] <- min(cvRm$cvm)
  min_measure[5,j] <- min(cvLm$cvm)
  min_measure[6,j] <- min(cvELm$cvm)
...
boxplot(ri, main = "Optimale Prognose Ridge, n = 100, N = 100, voll. Sep.", ylim = c(-8.5,1), ylab = "log(lambda)")
boxplot(la, main = "Optimale Prognose Lasso, n = 100, N = 100, voll. Sep.", ylim = c(-8.5,1), ylab = "log(lambda)")
boxplot(el, main = "Optimale Prognose Elastic Net 0.5, n = 100, N = 100, voll. Sep.", ylim = c(-8.5,1), ylab = "log(lambda)")
...
round(matrix(apply(min_measure,1,med),3,2),3)




stop("schöneres Beispiel")
